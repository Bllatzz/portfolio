...
const socialLinks = [
  {
    name: 'Instagram',
    url: 'https://instagram.com/bllatz',
    icon: (
  ...
  {
    name: 'LinkedIn',
    url: 'https://linkedin.com/in/joaopedrorochadev',
    icon: (
      <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
      ...
      <div className="container mx-auto px-6 relative z-10 text-center">
        ...
        >
          {socialLinks.map((link, index) => (
            <motion.a
              ...
              className={`group relative w-16 h-16 flex items-center justify-center rounded-full bg-dark-200/50 backdrop-blur-sm border border-white/10 text-gray-400 transition-all duration-300 ${link.color}`}
            ...
            >
              {link.icon}
              <span className="absolute -bottom-8 left-1/2 -translate-x-1/2 text-sm font-medium opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                {link.name}
              </span>
            </motion.a>
          ))}
        ...
        >
          <p className="text-gray-500 text-sm font-mono">
            © {new Date().getFullYear()} João Pedro Dias Rocha
          </p>
        </motion.div>
      </div>
    </section>
